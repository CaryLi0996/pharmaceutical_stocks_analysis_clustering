...
# Only keep the important numeric variables from 1-9:
...
# Euclidean distance is used as the most popular method (dissimilarity matrix)
...
# Plot silhoutte score for each k
...
# Find the closest clusters for each record
minSquaredDistances = distances.min(axis=1) ** 2

# Combine cluster labels into a data frame
df = pd.DataFrame({'squaredDistance': minSquaredDistances, 
   'cluster': kmeans.labels_}, index=num_df_norm.index)

# Group by clusters and print related info
...
# Average Distance: Hierarchical Clustering to examine the cluster groups to validate k-means clusters 
